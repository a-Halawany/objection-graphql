...
class SchemaBuilder {
  ...
  _resolverForModel(modelData, extraQuery) {
    return (ctx, ignore1, ignore2, data) => {
      ...
      if (ctx.args?.Function == undefined) {
        console.log('ss')
        ...
        const selectFilter = this._filterForSelects(ast, modelClass, data);
              ...
              var builder = modelClass.query(ctx.knex);
...
//     for (let i = 0, l = result.results.length; i < l; i += 1) {
//       console.log(result.results[i]+'hossam')
// result = []
//       result.push(result.results[i].$toJson());
//     }
//   }
  if (_.isArray(result)) {
