...
const { GraphQLObjectType, GraphQLSchema, GraphQLList , DirectiveLocation } = graphqlRoot;
      ...
      query: new GraphQLObjectType({
        ...
        fields: () => {
          ...
          _.forOwn(this.models, (modelData) => {
            ...
            const listFieldName = modelData.opt.listFieldName || (`${defaultFieldName}`);
  ...
  _filterForArgs(astNode, modelClass, variables) {
    ...
    return (builder) => {
      for (let i = 0, l = argObjects.length; i < l; i += 1) {
        ...
        if (!(typeof arg.value === 'undefined' && builder.internalOptions().skipUndefined) && arg.name != "Function") {
